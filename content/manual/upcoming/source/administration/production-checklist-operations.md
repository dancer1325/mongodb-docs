...
/administration/production-checklist-development list, provides
...
- Avoid using NFS drives for your `dbPath`.
  ...
  See: production-nfs for more information.
...
- Adjust the readahead settings on the devices
  ...
  - For the WiredTiger storage engine, set readahead between 8
    ...
    [MongoDB commercial support](https://support.mongodb.com/welcome) can provide
...
- If using `tuned` on RHEL / CentOS, you must customize your
  ...
  - Enable or disable Transparent Hugepages, depending on your MongoDB version.
    - If you are using MongoDB 8.0 or later, see using tuned and ktune to enable THP.
    - If you are using MongoDB 7.0 or earlier, see using tuned and ktune to disable THP.
...
- Disable NUMA or set vm.zone_reclaim_mode to 0 and run `mongod`
  instances with node interleaving. See: production-numa
  for more information.
...
- Adjust the `ulimit` values on your hardware to suit your use case. If
  ...
  accordingly. See: /reference/ulimit for more information.
...
- Ensure that the system default TCP keepalive is set correctly. A
  ...
  sharded clusters. See: faq-keepalive in the Frequently Asked
...
- Use [\|mms-home\|](##SUBST##|mms-home|) or Ops Manager, an on-premise solution available in MongoDB Enterprise Advanced or another monitoring system to
