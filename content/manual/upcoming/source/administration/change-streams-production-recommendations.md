...
that point in the oplog. Change stream cursors with the `fullDocument : updateLookup` option may return `null` for the lookup document.
...
For replica sets with arbiter members, change streams may remain
...
Use `rs.printReplicationInfo()` to retrieve information on the
...
response time of the change stream as the `mongos` must still check
...
`periodicNoopIntervalSecs` value.

If a sharded collection has high levels of activity, the `mongos`
...
For example, passing a `\$match` pipeline configured to filter
only `insert` operations.

## Indexes and Performance

Change streams cannot use indexes. MongoDB does not support creating
indexes on the oplog collection. Therefore, avoid opening a high
