* goal
  * how to use the `merge` aggregation stage 

* `merge` aggregation stage
  * allows
    * output pipeline results -- to -- a collection
  ...
  * - :ref:`into <merge-into>`
    ...
    - ..  _merge-into:
      ...
      - For a :ref:`replica set <replica-set>` or a standalone, if the output
        database does not exist, :pipeline:`$merge` also creates the database.
...
If all of the following are true for a :pipeline:`$merge` stage,
...
.. tabs-drivers::
   ...
   .. tab::
      ...
      The ``orgArchive`` collection has a unique compound index on the
      ``fiscal_year`` and ``dept`` fields. Specifically, there should be at
      ...
      First stage:
         ...
         To create the ``quarterlyreport`` collection, you can use this
         pipeline:
      ...
      .. tabs::
         ...
         .. tab:: Merge Stage
            ...
            - runs an :dbcommand:`aggregate` command that defines a
              ``year`` variable in the :pipeline:`$merge` :ref:`let
              <merge-let>` and adds the year to ``cakeSales`` using
         ...
         .. tab:: Aggregate Command
            ...
            - runs an :dbcommand:`aggregate` command that defines a
              ...
              ``cakeSales`` using :ref:`whenMatched
              <merge-whenMatched-pipeline>`
