...
* goal
  * MongoDB's read-only views' use cases

* MongoDB view
  * is a read-only queryable object whose contents are
...
- Create a {+fts+} index on a view, which
...
- Create a {+avs+} index on a view, which
