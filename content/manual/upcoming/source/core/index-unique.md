...
By default, MongoDB creates a unique index on the <span id="id">id</span> field when you create a new collection.
...
2.  Use the `collMod` command to set `prepareUnique` to
    `true` for the index.
...
3.  Resolve any existing duplicate values in the indexed field.

4.  Use the `collMod` command to make the index unique.
...
- Run `db.collection.createIndex()` on the primary for a
  replica set
- Run `db.collection.createIndex()` on the `mongos`
...
multikey index, a document may have array
...
partial-index-with-unique-constraints.
...
see shard-key-arbitrary-uniqueness.
