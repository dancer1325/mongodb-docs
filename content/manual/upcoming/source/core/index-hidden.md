...
- If a hidden index is a unique index, the
  index still applies its unique constraint to the documents.
- If a hidden index is a TTL index, the index
  still expires documents.
- Hidden indexes are included in `listIndexes` and
  `db.collection.getIndexes()` results.
- Hidden indexes are updated upon write operations to the collection
  ...
  `db.collection.stats()` and `\$indexStats`.
- Hiding an unhidden index or unhiding a hidden index resets its
  `\$indexStats`. Hiding an already hidden index or unhiding
  an already unhidden index does not reset the `\$indexStats`.

## Restrictions

- To hide an index, you must have featureCompatibilityVersion set to `{+minimum-lts-version+}` or greater.
...
`db.collection.createIndex()` method with the hidden option set to `true`.
...
To verify, run `db.collection.getIndexes()` on the
...
- the index key specification document to the
  `db.collection.hideIndex()` method:
...
or `mongosh` helper
`db.collection.unhideIndex()`. You can specify either:

- the index key specification document to the
  `db.collection.unhideIndex()` method:
