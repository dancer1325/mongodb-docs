...
This tutorial shows you how to create a local Atlas deployment with Docker. In this tutorial, we will deploy a single-node replica set with Docker. For a complete and detailed Docker Compose example, see atlas-cli-docker-compose.
...
1. **Pull down the `mongodb/mongodb-atlas-local` Docker image.**
...
1. **Run the Docker image.**
   ...
   **Automate Connection**
   ...
   To automate a containerized deployment of Atlas, you'll need to wait for the container to be in a healthy state before you can connect to Atlas. The following example demonstrates deploying an Atlas image in Docker and polling Docker to check the state of the container. Once the container is in a healthy state, the script automates a connection to your Atlas instance with [Mongosh](https://www.mongodb.com/docs/mongodb-shell/install/).
...
1. **Connect to the local Atlas deployment.**
...
Create a local Atlas deployment with [Docker Compose](https://docs.docker.com/compose/). For a complete and detailed example, see atlas-cli-docker-compose.

> **Note:**
> If you have an existing Docker-Compose based Atlas implementation that you've built with the official [MongoDB Docker Image](https://hub.docker.com/_/mongo) and that you'd like to convert to use the Atlas Local Dev image, see Convert to a Local Dev Implementation below.
...
1. **Install Docker Compose.**
   ...
   **Example:**
   ...
   To learn more, see the [Docker Compose install documentation](https://docs.docker.com/compose/install/).

1. **Create a `docker-compose.yaml` file.**

   Create the `docker-compose.yaml` file in the same directory that you run Docker Compose from. For a complete and detailed example, see atlas-cli-docker-compose. **Example:**
...
1. **Run Docker Compose.**
   ...
   The following command creates a local Atlas deployment with Atlas Search capabilities enabled. **Example:**
...
You can persist data across multiple runs with [Docker Compose](https://docs.docker.com/compose/). Persisting data helps to ensure that data isn't lost between runs. Data remains available across runs of Docker Compose.
...
1. **Create a `docker-compose.yaml` file.**
   ...
   Update the `docker-compose.yaml` file to mount the necessary data directories as volumes. **Example:**
...
1. **Run Docker Compose.**
   ...
   You can also run Docker Compose in [detached mode](https://docs.docker.com/reference/cli/docker/compose/up/#options). **Example:**
...
1. **Install syft.**
   ...
   **Example:**
   ...
   To learn more, see the [syft README](/anchore/syft/blob/main/README.md).
...
1. **Install cosign.**
   ...
   **Example:**
   ...
   To learn more, see the [cosign Installation](https://docs.sigstore.dev/system_config/installation/).
...
To run the `mongodb/mongodb-atlas-local` Docker image with GitHub actions, create a workflow file. To learn more, see the [GitHub Actions Quickstart](https://docs.github.com/en/actions/quickstart). **Example:** Create the following `mongodb.yml` file in the `.github/workflows` directory:
...
If you have an existing Atlas implementation running in Docker Compose that you have built with the official [mongo](https://hub.docker.com/_/mongo) Docker image, you can refer to the following checklist to simplify converting it to the [mongodb-atlas-local](https://hub.docker.com/r/mongodb/mongodb-atlas-local) image.
