:template: drivers-index
:hidefeedback: header
:noprevnext:

=============================
Start Developing with MongoDB
=============================

* goal
  * how to connect your application -- , via MongoDB client libraries, to -- 
    * MongoDB Atlas deployment, OR
    * self-hosted MongoDB cluster

* AVAILABLE | multiple
  * programming language
  * frameworks
   
* documentation + API / EACH client library

You can connect your application to your MongoDB Atlas deployment or a
self-hosted MongoDB cluster by using one of the official MongoDB
libraries. To learn more about Atlas, see :atlas:`What is MongoDB Atlas? </>`. To learn how to download and install a self-hosted
MongoDB cluster, see :manual:`Install MongoDB </installation>`.

The following libraries are officially supported by MongoDB. MongoDB actively
develops new features, adds performance enhancements, fixes bugs, and applies 
security patches to them.

.. card-group::
   :columns: 3
   :layout: default
   :type: drivers

   .. card::
      :headline: C
      :url: https://www.mongodb.com/docs/languages/c/
      :icon: /icons/c.svg
      :icon-alt: C Driver icon

   .. card::
      :headline: C++
      :url: https://www.mongodb.com/docs/languages/cpp/
      :icon: /icons/c++.svg
      :icon-alt: C plus plus Driver icon

   .. card::
      :headline: C#
      :url: https://www.mongodb.com/docs/languages/csharp/
      :icon: /icons/csharp.svg
      :icon-alt: C sharp Driver icon

   .. card::
      :headline: Go
      :url: https://www.mongodb.com/docs/languages/go/
      :icon: /icons/go.svg
      :icon-alt: Go Driver icon

   .. card::
      :headline: Java
      :url: https://www.mongodb.com/docs/languages/java/
      :icon: /icons/java.svg
      :icon-dark: /icons/java-dark.svg
      :icon-alt: Java Driver icon
   
   .. card::
      :headline: Kotlin
      :url: https://www.mongodb.com/docs/languages/kotlin/
      :icon: /icons/kotlin.svg
      :icon-alt: Kotlin Driver icon
   
   .. card::
      :headline: Node.js
      :url: https://www.mongodb.com/docs/languages/javascript/
      :icon: /icons/node.svg
      :icon-dark: /icons/node-dark.svg
      :icon-alt: Node Driver icon

   .. card::
      :headline: PHP
      :url: https://www.mongodb.com/docs/languages/php/
      :icon: /icons/php.svg
      :icon-alt: PHP Driver icon

   .. card::
      :headline: Python
      :url: https://www.mongodb.com/docs/languages/python/
      :icon: /icons/python.svg
      :icon-alt: Python Driver icon
   
   .. card::
      :headline: Ruby
      :url: https://www.mongodb.com/docs/languages/ruby/
      :icon: /icons/ruby.svg
      :icon-alt: Ruby Driver icon

   .. card::
      :headline: Rust
      :url: https://www.mongodb.com/docs/languages/rust/
      :icon: /icons/rust.svg
      :icon-dark: /icons/rust-dark.svg
      :icon-alt: Rust Driver icon

   .. card::
      :headline: Scala
      :url: https://www.mongodb.com/docs/languages/scala/
      :icon: /icons/scala.svg
      :icon-alt: Scala Driver icon

   .. card::
      :headline: Swift
      :url: /swift/
      :icon: /icons/swift.svg
      :icon-alt: Swift Driver icon

   .. card::
      :headline: TypeScript
      :url: https://www.mongodb.com/docs/languages/javascript/
      :icon: /icons/ts.svg
      :icon-alt: TypeScript Driver icon


Featured Community-Supported Libraries
-----------------------------------------

The following libraries are developed and maintained by the MongoDB community. 
They are not official MongoDB drivers and are not supported by MongoDB. 
For questions, issues, or updates, please refer to the respective project 
repositories and maintainers.

.. card-group::
   :columns: 3
   :layout: carousel
   :type: drivers

   .. card::
      :cta: Elixir
      :url: https://github.com/zookzook/elixir-mongodb-driver
      :icon: /icons/elixir.svg
      :icon-alt: Elixir icon

   .. card::
      :cta: Mongoose
      :url: https://mongoosejs.com/docs/index.html
      :icon: /icons/mongoose.svg
      :icon-alt: Mongoose icon
   
   .. card::
      :cta: Prisma
      :url: https://www.prisma.io/docs/guides/database/mongodb
      :icon: /icons/prisma.svg
      :icon-dark: /icons/prisma-dark.svg
      :icon-alt: Prisma icon

   .. card::
      :cta: R
      :url: https://cran.r-project.org/web/packages/mongolite/
      :icon: /icons/r.svg
      :icon-alt: R icon

Don't see your desired language? Browse more `community-supported libraries <https://github.com/ramnes/awesome-mongodb#libraries>`__.

Compatibility with Other Document Databases
-------------------------------------------

See the following pages for information about other document databases' compatibility
with MongoDB:

- :ref:`documentdb-compat`
- :ref:`cosmosdb-compat`

.. toctree::

   C Driver <https://www.mongodb.com/docs/languages/c/c-driver/current/>
   C++ Driver <https://www.mongodb.com/docs/languages/cpp/cpp-driver/current/>
   .NET/C# Driver </csharp-drivers>
   Go Driver <https://www.mongodb.com/docs/drivers/go/current/>
   Java Drivers </java-drivers>
   Kotlin Drivers </kotlin-drivers>
   Node.js Driver </node-drivers>
   PHP Drivers </php-drivers>
   Python Drivers </python-drivers>
   Ruby Drivers  </ruby-drivers>
   Rust Driver </rust-drivers>
   Scala Driver <https://www.mongodb.com/docs/languages/scala/scala-driver/current/>
   Swift Driver </swift>
   TypeScript </typescript>
   Compatibility </compatibility>
   Other Document Database Compatibility </other-document-dbs>
